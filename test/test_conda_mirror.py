...
def repodata():
    ...
    repodata[anaconda_channel] = conda_mirror.get_repodata(anaconda_channel, "linux-64")
    return repodata
...
def test_match(repodata):
    ...
    matched = conda_mirror._match(
        repodata_packages, dict(name="python", version=">=3.7,<3.8")
    )
...
whitelist:
    - name: {}
      version: {}""".format(
            packages[smallest_package]["name"], packages[smallest_package]["version"]
        )
    )
    cli_args = (
        "conda-mirror"
    ...
    _write_bad_package(
        channel_dir=f2.strpath, platform_name=platform, pkg_name="bad-1-0.tar.bz2"
    )
    ...
    _write_bad_package(
        channel_dir=f2.strpath, platform_name=platform, pkg_name=upstream_pkg_name
    )
    conda_mirror.cli()
    sys.argv = old_argv
    ...
    with bz2.BZ2File(os.path.join(f2.strpath, platform, "repodata.json.bz2"), "r") as f:
...
def test_main(tmpdir, repodata):
    ...
    ret = conda_mirror.main(
        ...
        whitelist=[
            {
                "name": packages[smallest_package]["name"],
    ...
    assert (
        len(ret["validating-existing"]) == 0
    ), "There should be no already-downloaded packages"
    ...
    ret = conda_mirror.main(
        ...
        whitelist=[
            {
                "name": packages[next_smallest_package]["name"],
...
def test_dry_run_dumb(tmpdir):
    ...
    assert (
        len(ret["to-mirror"]) > 1
    ), "We should have a great deal of packages slated to download"
